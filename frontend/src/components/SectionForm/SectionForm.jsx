...
import { GetSectionDataByCourseCode } from '../../api.jsx';
...
export default function SectionForm() {
    ...
    const { selectedCourse, isCourseSelected } = useCourse();
    ...
    const [bgColour, setBgColour] = useState("#87CEEB");
    ...
    useEffect(() => {
        if (isCourseSelected) {
            const fetchDropdownValues = async (course) => {
                try {
                    const values = await GetSectionDataByCourseCode(course);
    ...
    const handleSelect = (rowData) => {
        const { courseTitle, type, days, startTime, endTime, location } = rowData;
    ...
    return (
        <div className='course-and-section-form-container'>
            ...
            <Button id='section-button' label='Select a LEC/LAB section' severity="info" disabled={!isCourseSelected} onClick={() => setDialogVisible(true)}/>
            ...
            <Button id="add-course-button" label="Add Course" severity="info" raised disabled={disabled} onClick={formCheck}/>
        </div>
    );
}
